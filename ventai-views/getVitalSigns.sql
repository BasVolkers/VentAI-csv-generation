...
DROP MATERIALIZED VIEW IF EXISTS getVitalsigns2 ;
CREATE MATERIALIZED VIEW getVitalsigns2 as
...
select
  ...
  from ce
  LEFT JOIN getGCS2 gcs 
  ON (ce.icustay_id = gcs.icustay_id)
  AND (ce.charttime = gcs.charttime)
  group by ce.subject_id,ce.hadm_id,ce.icustay_id, ce.charttime, gcs.gcs
