...
DROP MATERIALIZED VIEW IF EXISTS ws_getVentilationData;
CREATE MATERIALIZED VIEW ws_getVentilationData AS

-- Extracts from chartevents all data related to ventilation
-- Outputs a table that has rows with: subject_id, hadm_id, icustay_id, charttime, FiO2, PEEP, tidal_volume, plateau_pressure, mechvent
-- Will have many rows where mechvent = 0 and hence filters mechvent = 1 in the end
...
select
  ce.icustay_id, ce.subject_id, ce.hadm_id, ce.charttime
	...
	FROM chartevents ce WHERE ce.value is not null AND ce.error IS DISTINCT FROM 1 AND ce.itemid in
...
-- STEP 3: Get mechanical ventilation
  ...
  from chartevents ce 
  JOIN data_ ON (data_.icustay_id = ce.icustay_id) AND (data_.charttime = ce.charttime)
  where ce.value is not null and ce.error IS DISTINCT FROM 1
  group by ce.subject_id, ce.hadm_id, ce.icustay_id, ce.charttime
  ORDER BY ce.subject_id, ce.hadm_id, ce.icustay_id, ce.charttime
)

select base.*, 
  plateau_pressure - peep as driving_pressure,
  base.tidal_volume / ibw.ideal_body_weight as adjusted_tidal_volume
from base 
JOIN getIdealBodyWeight ibw on (ibw.icustay_id = base.icustay_id);
-- where mechvent = 1;