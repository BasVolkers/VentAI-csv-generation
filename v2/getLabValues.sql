...
DROP MATERIALIZED VIEW IF EXISTS ws_getLabvalues;
CREATE table ws_getLabvalues AS
...
(
  ...
  from chartevents ce where ce.error IS DISTINCT FROM 1 and ce.itemid in
  (3801,3802, -- SGOT and SGPT
   816,1350,3766,8177,8325,225667 -- Ionized Calcium
)),
others as(
  select subject_id, hadm_id, ce.icustay_id, ce.charttime, avg(SGOT) as SGOT, avg(SGPT) as SGPT, avg(IonizedCalcium) as IonizedCalcium
  from ce
  group by ce.subject_id,ce.hadm_id,ce.icustay_id, ce.charttime
)

  select
  	le.subject_id as subject_id, le.hadm_id as hadm_id, le.icustay_id as icustay_id, le.charttime as charttime, avg(ALBUMIN) as ALBUMIN , avg(ANIONGAP) as ANIONGAP , 
  ...
  avg(others.SGOT) as SGOT, avg(others.SGPT) as SGPT, avg(others.IonizedCalcium) as IonizedCalcium
  from le FULL JOIN others ON (le.icustay_id = others.icustay_id) AND (le.charttime = others.charttime)
  group by le.icustay_id,le.subject_id,le.hadm_id,le.charttime 
  order by le.icustay_id,le.subject_id,le.hadm_id,le.charttime;
